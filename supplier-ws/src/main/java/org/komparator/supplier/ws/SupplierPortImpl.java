...
public class SupplierPortImpl implements SupplierPortType {
	...
	public ProductView getProduct(String productId) throws BadProductId_Exception {
		...
		messageContext.put(AuthenticationHandler.SENDER_NAME, this.endpointManager.getWSName());
	...
	public List<ProductView> searchProducts(String descText) throws BadText_Exception {
		...
		for (Product p : supplier.searchProducts(descText)) {
			ProductView pv = newProductView(p);
			pvs.add(pv);
		}
	...
	public String buyProduct(String productId, int quantity)
		...
		if(quantity <= 0) {
			throwBadQuantity("Product quantity cannot be lesser or equal than zero!");
		}
		...
		try {			
			...
			throwInsufficientQuantity("There is no such product quantity at the supplier!");
			return null;
		}
